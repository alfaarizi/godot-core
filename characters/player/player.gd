...
func _update_player_direction(_door: Door) -> void:
		...
		cutscene_sprite_2d.animation = anim_name
...
func _physics_process(_delta: float) -> void:		
	if cutscene_sprite_2d.is_animation_running:
		return
	...
	cutscene_sprite_2d.play(_move_action)
	
	if direction.length() > 0.0:	
		cutscene_sprite_2d.flip_h = direction.x < 0.0
		var current_speed_percent := velocity.length() / _current_max_speed
		cutscene_sprite_2d.speed_scale = 1.75 if current_speed_percent > running_speed_percent else 1.0

...
@export var cutscene_items: Array[CutsceneItem] = []: set = _set_cutscenes
var cutscene_dict: Dictionary = {}

func _ready() -> void:
	if Engine.is_editor_hint(): return
	for ci in self.cutscene_items:
		if ci and ci.cutscene_name:
			self.cutscene_dict[ci.cutscene_name] = ci

func play_cutscene(cutscene_name: String):
	if self.cutscene_dict.is_empty(): return

	var curr_ci: CutsceneItem = self.cutscene_dict.get(cutscene_name, null)
	if not curr_ci: return

	GameState.change_state(GameState.State.CUTSCENE)
	for cf in curr_ci.frames:
		await _execute(cf)
	GameState.change_state(GameState.State.EXPLORING)

func _execute(cutscene_frame: CutsceneFrame):
	if not cutscene_frame: return

	var action := cutscene_frame.action
	var action_type := cutscene_frame.action_type
...
func _set_cutscenes(_cutscenes: Array[CutsceneItem]):
	for idx in _cutscenes.size():
		if _cutscenes[idx] == null:
			_cutscenes[idx] = CutsceneItem.new()
	cutscene_items = _cutscenes
	update_configuration_warnings()

func _get_configuration_warnings() -> PackedStringArray:
	if cutscene_items.is_empty():

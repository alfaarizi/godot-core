...
func _show_text(dialogue_item_idx: int) -> void:
	...
	dialogue_tween.finished.connect(func() -> void:
		...
		arrow_tween.finished.connect(on_finished)
	)
...
# SLIDE ANIMATIONS
func _slide(slide_type: SlideDirection,
		container: MarginContainer,
		margin_front: String,
		margin_behind: String,
		distance: int,
		duration: float
	) -> void:
...
func _character_slide(slide_type: SlideDirection, character: Profile, duration: float) -> void:
	...
	if character == player:
		_slide(slide_type, character.container, margins[0], margins[1], 100, duration)
	elif character == npc:
		_slide(slide_type, character.container, margins[1], margins[0], 100, duration)

func _dialogue_slide(slide_type: SlideDirection, duration: float) -> void:
	var margins: Array[String] = []
	match slide_type:
		SlideDirection.SLIDE_IN:
	...
	_slide(slide_type, dialogue_container, margins[0], margins[1], 250, duration)
